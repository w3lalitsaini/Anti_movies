...
const MovieCard = ({ movie }) => {
  // 1. Define the identifier (Use slug if it exists, otherwise fallback to _id)
  const movieIdentifier = movie.slug || movie._id;

  return (
    /* 2. Update the 'to' path here */
    <Link
      to={`/movie/${movieIdentifier}`}
      className="movie-card group relative rounded-xl overflow-hidden bg-secondary border border-neutral-800 hover:border-primary transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-primary/20"
    >
      <div className="relative aspect-[2/3] overflow-hidden">
        <img
          src={
            movie.poster ||
            `https://via.placeholder.com/300x450/1a1a1a/e50914?text=${encodeURIComponent(movie.title)}`
          }
          alt={movie.title}
